...
describe("App", () => {
  it('render App component', async() => {
    ...
    expect(await screen.findByText('Hello! Sorry, there is nothing special here yet, but it may appear later')).toBeInTheDocument();

    expect(screen.getByText('About Us')).toHaveClass('active');
    fireEvent.click(screen.getByText('Posts Page'));
    expect(screen.queryByText('Hello! Sorry, there is nothing special here yet, but it may appear later')).toBeNull();
  })
})
